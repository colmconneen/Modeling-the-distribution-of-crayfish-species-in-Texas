...
full.model.PCNM.s <- lrm(presence ~ PCNM1 + PCNM2 + PCNM3 #+ PCNM4 + PCNM5 + PCNM6 + PCNM7 + PCNM8 + PCNM9 + PCNM10 + PCNM11 + PCNM12 + PCNM13 + PCNM14 
                       +pca_1
                       +pca_2
                       +pca_3
                       +pca_4
...
model.perf.res.s <- c(avg.auc,avg.auc.diff,avg.auc.train,avg.cor,avg.deviance,avg.kappa,avg.tss)  
...
d.f.s <- as.data.frame(t(model.perf.res.s))
colnames(d.f.s) <- colnam

# Write to file
write.table(d.f.s, file = "Simulans model performance res.txt", row.names = FALSE, col.names = TRUE)


############### generating plots of habitat suitability versus each principle component
# Assume your model is called `model`
# Assume your principal components are stored in a data frame called `pc_data`
# Example: model <- glm(habitat_suitability ~ PC1 + PC2 + PC3, data = pc_data, family = binomial)
...
# Example for PC1
plot_response_curve_ggplot(full.model.PCNM.s, species_data, "pca_1")

plot_response_curve_ggplot(full.model, species_data, "pca_1")
...
res.pcnm.s.model <- residuals(full.model.PCNM.s, type = "pearson")
res.s.model <- residuals(full.model.s, type = "pearson")

# coordinates
coords.s <- cbind(species_data$X, species_data$Y)
...
knn.s <- knearneigh(coords.s, k = 5)
nb.s <- knn2nb(knn.s)

# spatial weights
lw.s <- nb2listw(nb.s, style = "W")

# Moran's I test
moran.I.s.w.pcnm <- moran.test(res.pcnm.s.model, lw.s)
moran.I.s <- moran.test(res.s.model,lw.s)
